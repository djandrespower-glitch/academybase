// ============================================================
//  DEEJAY ACADEMY — script.js con Firebase Firestore
// ============================================================
...
var DB = {
  alumnos:[], pagos:[], cuotas:[], asistencias:[],
  cursos:[], horario_grupos:[], aulas_vinculados:[]
};
...
// ── Login ───────────────────────────────────────────────────
function checkLogin() {
  if (sessionStorage.getItem('dja_auth') === '1') return true;
  ...
  ov.innerHTML = '<div style="background:#fff;border-radius:16px;padding:36px 32px;width:300px;text-align:center">'
    ...
    + '<input id="_lpwd" type="password" placeholder="Contraseña" style="width:100%;padding:10px 14px;border:1.5px solid #e0e0e0;border-radius:10px;font-size:15px;outline:none;text-align:center;letter-spacing:4px;margin-bottom:12px">'
  ...
  document.getElementById('_lpwd').addEventListener('keydown', function(e){ if(e.key==='Enter') doLogin(); });
  document.getElementById('_lpwd').focus();
  return false;
}
...
window.doLogin = function() {
  ...
  if (pwd === '4003') {
    sessionStorage.setItem('dja_auth', '1');
    document.getElementById('_login').remove();
    initApp();
  } else {
    document.getElementById('_lerr').textContent = 'Contraseña incorrecta';
...
function initApp() {
  // Cursos por defecto si Firestore está vacío
  ...
  listenCol("cuotas",         "cuotas",         function(){ renderDash(); updBadge(); renderCuotas(); });
  ...
  listenCol("horario_grupos", "horario_grupos", function(){ if(document.getElementById('page-horarios_aulas').classList.contains('active')) renderHorariosPage(); });
}
...
function confirmDel(msg,cb){
  ...
  ov.innerHTML='<div style="background:#fff;border-radius:12px;padding:24px;max-width:320px;width:90%;text-align:center">'
    ...
    +'<button id="_cds" style="padding:8px 20px;border-radius:8px;border:none;background:#e8c547;color:#1a1a2e;cursor:pointer;font-size:13px;font-weight:600">Eliminar</button>'
    +'</div></div>';
  document.body.appendChild(ov);
...
window.showPage = function(id,el){
  ...
  var ac=document.getElementById('topbar-acts');ac.innerHTML='';
...
function calcAlerts(){
  var al=[];
  DB.cuotas.forEach(function(c){var a=gA(c.alumnoId);if(a)al.push({t:a.nombre+' - $'+parseFloat(c.monto||0).toLocaleString('es-CO')+' ('+(c.descripcion||'Mensualidad')+')'})});
  return al;
}
...
function renderAlertas(){
  ...
  el.innerHTML='<div style="padding:12px 14px;font-weight:600;font-size:13px;border-bottom:1px solid #f0f0f0">'+als.length+' alerta(s)</div>'+als.map(function(a){return'<div class="ai"><span style="color:#b91c1c;font-weight:500">'+a.t+'</span></div>'}).join('');
  updBadge();
}
...
window.openMAl = function(id){
  ...
  document.getElementById('p2-hist').innerHTML='';document.getElementById('cuotas-lista').innerHTML='<div style="color:#aaa;font-size:12px;padding:6px">Sin cuotas.</div>';
...
window.saveAlumno = async function(){
  ...
  if(eAid){
    await fbUpd('alumnos',eAid,data);
  } else {
    data.creado=new Date().toISOString().split('T')[0];
    eAid=await fbAdd('alumnos',data);
  }
...
window.doAlDel=function(){
  ...
  confirmDel('¿Eliminar este alumno?',async function(){
...
// ── Cuotas ──────────────────────────────────────────────────
window.addCuota = async function(){
  if(!eAid){alert('Guarda el alumno primero.');return}
  await fbAdd('cuotas',{alumnoId:eAid,descripcion:'',monto:'',vencimiento:'',forma:'Efectivo',creado:new Date().toISOString().split('T')[0]});
  renderCuotas();
}
...
function renderCuotas(){
  ...
  if(!cuotas.length){el.innerHTML='<div style="color:#aaa;font-size:12px;padding:6px">Sin cuotas.</div>';return}
  el.innerHTML=cuotas.map(function(c){return'<div style="display:grid;grid-template-columns:1fr 100px 120px 100px auto;gap:5px;align-items:center;margin-bottom:6px;padding:8px;background:#fffdf0;border:1px solid #fde68a;border-radius:8px"><input style="font-size:12px;padding:5px 8px;border:1px solid #e0e0e0;border-radius:6px" placeholder="Concepto" value="'+(c.descripcion||'').replace(/"/g,'&quot;')+'" onchange="updCuota(\''+c.id+'\',\'descripcion\',this.value)"><input type="number" style="font-size:12px;padding:5px 8px;border:1px solid #e0e0e0;border-radius:6px" placeholder="Monto" value="'+(c.monto||'')+'" onchange="updCuota(\''+c.id+'\',\'monto\',this.value)"><input type="date" style="font-size:12px;padding:5px 8px;border:1px solid #e0e0e0;border-radius:6px" value="'+(c.vencimiento||'')+'" onchange="updCuota(\''+c.id+'\',\'vencimiento\',this.value)"><select style="font-size:12px;padding:5px 8px;border:1px solid #e0e0e0;border-radius:6px" onchange="updCuota(\''+c.id+'\',\'forma\',this.value)"><option value="Efectivo" '+(c.forma==='Efectivo'?'selected':'')+'>Efectivo</option><option value="Llave" '+(c.forma==='Llave'?'selected':'')+'>Llave</option><option value="Tarjeta" '+(c.forma==='Tarjeta'?'selected':'')+'>Tarjeta</option></select><div style="display:flex;gap:3px"><button class="btn bo bsm" style="font-size:11px;color:#a16207" onclick="regCuota(\''+c.id+'\')">Cobrar</button><button class="btn bd bsm" style="font-size:11px" onclick="delCuota(\''+c.id+'\')">X</button></div></div>'}).join('');
}

window.updCuota=async function(id,f,v){await fbUpd('cuotas',id,{[f]:v})}
window.delCuota=async function(id){await fbDel('cuotas',id);renderCuotas();if(eAid)renderHistP(eAid);}
window.regCuota=async function(id){
  var c=DB.cuotas.find(function(x){return x.id===id});
  if(!c||!parseFloat(c.monto)){alert('Ingresa el monto primero.');return}
  await fbAdd('pagos',{alumnoId:c.alumnoId,periodo:c.descripcion||'Cuota',forma:c.forma,monto:parseFloat(c.monto),estado:'Pagado',fecha:c.vencimiento||new Date().toISOString().split('T')[0],creado:new Date().toISOString().split('T')[0]});
  await fbDel('cuotas',id);
  renderCuotas();if(c.alumnoId)renderHistP(c.alumnoId);
}
...
function renderHorariosPage(){
  if(!window.hCursoTab&&DB.cursos.length) window.hCursoTab=DB.cursos[0].id;
  var mesAct=window.hMes;
  var cursoAct=window.hCursoTab;
  ...
  var tabs=DB.cursos.map(function(c){
    var act=c.id===cursoAct;
    return'<button onclick="window.hCursoTab=\''+c.id+'\';renderHorariosPage()" style="padding:6px 16px;border-radius:8px;cursor:pointer;font-size:12px;font-weight:600;border:none;background:'+(act?'#e8c547':'#1f2937')+';color:'+(act?'#111':'#9ca3af')+'">'+c.nombre+'</button>';
  }).join('');
    ...
    var rows=FRANJAS_H.map(function(fr){
      var tds=niveles.map(function(n,ni){
        ...
        var cards=grupos.map(function(g){
          ...
          return'<div onclick="editGrupo(\''+g.id+'\')" style="border-radius:10px;padding:8px 10px;margin-bottom:6px;position:relative;background:rgba(255,255,255,.09);border:1px solid rgba(255,255,255,.15);cursor:pointer">'
            +'<button onclick="event.stopPropagation();delGrupo(\''+g.id+'\')" style="position:absolute;top:4px;right:4px;background:#c0392b;border:none;border-radius:50%;width:18px;height:18px;font-size:10px;cursor:pointer;color:#fff;line-height:18px;text-align:center;padding:0">x</button>'
        ...
        return'<td style="background:'+bg+';border-radius:10px;padding:7px;vertical-align:top;min-height:90px">'+cards
          +'<div onclick="openGrupoModal(\''+cursoAct+'\',\''+n+'\',\''+fr+'\')" style="border:1.5px dashed rgba(255,255,255,.25);border-radius:8px;padding:7px;text-align:center;cursor:pointer;font-size:12px;color:rgba(255,255,255,.35);margin-top:3px">+ grupo</div></td>';
      }).join('');
  ...
  document.getElementById('horarios-root').innerHTML=
    ...
    +'<select onchange="window.hMes=parseInt(this.value);renderHorariosPage()" style="background:#1f2937;color:#fff;border:1px solid #374151;border-radius:8px;padding:6px 12px;font-size:13px;outline:none;font-family:inherit">'+mesOpts+'</select>'
    +'<span style="color:#9ca3af;font-size:12px;margin-left:6px">inicia</span>'
    +'<input type="date" value="'+fIni+'" onchange="setCursoFecha(\'inicio\',this.value)" style="background:#1f2937;color:#fff;border:1px solid #374151;border-radius:8px;padding:6px 10px;font-size:13px;outline:none;width:140px">'
    +'<span style="color:#9ca3af;font-size:12px">termina</span>'
    +'<input type="date" value="'+fFin+'" onchange="setCursoFecha(\'fin\',this.value)" style="background:#1f2937;color:#fff;border:1px solid #374151;border-radius:8px;padding:6px 10px;font-size:13px;outline:none;width:140px">'
    +'</div>'
    +'<div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:14px">'+tabs+'</div>'
    +gridHtml+'</div>';
}

window.renderHorariosPage=renderHorariosPage;
window.setCursoFecha=async function(campo,val){
  if(!window.hCursoTab) return;
  // Guarda fecha por curso+mes: fechas.mes_N.inicio / fechas.mes_N.fin
  var key = 'fechas_mes_'+window.hMes+'_'+campo;
  await fbUpd('cursos', window.hCursoTab, {[key]: val});
}
...
window.expJSON = function(){
  var backup = {
    ...
    cursos:         DB.cursos,
    horario_grupos: DB.horario_grupos
  };
  var blob = new Blob([JSON.stringify(backup, null, 2)], {type:'application/json'});
  var a = document.createElement('a');
  var fecha = new Date().toISOString().split('T')[0];
  a.href = URL.createObjectURL(blob);
  a.download = 'DJA_Backup_'+fecha+'.json';
  a.click();
}
  ...
  input.onchange = function(e){
    ...
    reader.onload = function(ev){
      try {
        ...
        if(!data.alumnos && !data.cursos){ alert('Archivo no válido.'); return; }
        ...
        if(!confirm('¿Importar '+total+' registros a Firestore?\n\nEsto AGREGA los datos al proyecto actual. Los registros existentes NO se eliminan.\n\nFecha del backup: '+(data._fecha||'desconocida'))) return;
        importarJSON(data);
...
async function importarJSON(data){
  var el = document.getElementById('_imp_status');
  if(el) el.textContent = 'Importando...';
  ...
  for(var ci=0; ci<cols.length; ci++){
    ...
    for(var i=0; i<rows.length; i++){
      ...
      var id = row.id; delete row.id; delete row._ts;
      try {
        if(id) await fbSet(col, id, row);
        else    await fbAdd(col, row);
      } catch(err){ console.warn('Error importando '+col, err); }
      done++;
      if(el) el.textContent = 'Importando... '+done+'/'+total;
    }
  }
